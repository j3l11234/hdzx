...
var timeSlide;
$(function() {
	$('.roomitem:even').css('background', '#FFF');
	var delay = false;
...
function search() {
	return true;
	var p = $('#searchbar').serializeArray();
	<?php
 $xparam = array( 'name' => '$name$', 'date' => '$date$', 'start' => '$start$', 'end' => '$end$', 'floor' => '$floor$', 'type' => '$type$' ); ?>
	var url = '<?php echo U('index', $xparam);?>';
	for(i in p) {
		if(!p[i].value)
			p[i].value = 'false';
		url = url.replace('$' + p[i].name + '$', p[i].value);
	}
	document.location.href = url;
	return false;
}
</script><div class="float-fix"><form id="searchbar" action="<?php echo U('index');?>" method="get" onsubmit="return search()" class="nav-bar" style="float: right; width: 30%; margin-right: 10px;"><div class="nav-bar2"><div class="nav-bar3"><a class="active">
					房间名称或房间号检索
				</a><div class="tool-box"><label>房间名/号:</label><input type="text" class="field" style="width:150px" name="name" value="<?php echo ($param["name"]); ?>"/></div><a class="active">
					房间可预约时间检索
				</a><div class="tool-box"><label>日期:</label><input type="text" name="date" class="field datepicker" readonly style="width: 150px;" value="<?php echo ($param["date"]); ?>"/><a href="javascript:void(0)" onclick="$('.datepicker').val('')">清除</a><br/><label>时段：</label><input type="text" class="plaintext" name="start" id="timestart" value="<?php echo ($param["start"]); ?>" onchange="timeChange()"/>
					点 到
					<input type="text" class="plaintext" name="end" id="timeend" value="<?php echo ($param["end"]); ?>" onchange="timeChange()"/>
					点
					<div class="slider" style="margin: 10px; z-index: 0"></div></div><a class="active">
					房间类别检索
				</a><div class="tool-box" style="padding: 10px"><label>类型:</label><select name="type"><option value="0">任意</option><?php echo options($roomTypes, $param['type']);?></select>
					&nbsp;&nbsp;&nbsp;
					<label>楼层:</label><select name="floor"><option value="0">任意</option><?php echo options($floors, $param['floor']);?></select></div><div class="hr"></div><div class="tool-box" style="text-align: center"><button class="btn" onclick="search()">按以上条件检索</button><br/><i>表格中未填写内容不参与检索</i></div></div></div></form><div style="float: left; width: 650px; margin-left: 25px; position: relative"><div class="timelist float-fix"><table cellspacing="0" cellpadding="0"><tr><td style="width: 150px; text-align: center"><i style="font-size: 10px; color: #FFF">拖动此轴以查看更多时间</i></td><td style="width: 500px;"><div class="timelist2"><ul class="timelined"><?php
 $rdates = array(); foreach($dates as $day) { $rdates[] = str_replace('-', '', substr($day, 0, 10)); ?><li><?php echo ($day); ?></li><?php } ?></ul></div></td></tr></table></div><div id="timedrag" class="timelist float-fix">&nbsp;</div><?php $xparam = $param; $xparam['page'] = ''; $xparam = U('index') . '?' . http_build_query($xparam) . '$$';?><div class="pager" style="padding: 5px; padding-top:40px;"><?php echo show_pager($pageTotal, $param['page'], $xparam);?></div><?php foreach($rooms as $room) { ?><div class="float-fix roomitem"><table style="650px" cellspacing="0" cellpadding="0"><tr><td style="color: #222; width: 150px; text-align: center"><strong><?php echo ($room["name"]); ?></strong><br/><i><?php echo ($room["number"]); ?></i><br/><br/><a href="<?php echo U('Index/room', array(id=>$room['roomid']));?>">房间详情</a></td><td style="width: 500px;"><div style="width: 500px; overflow: hidden"><table class="timelined" style="width: 1500px" cellspacing="0" cellpadding="0"><tr><?php for($m=0; $m<15; $m++){ ?><td><a class="timecell" href="<?php echo U('order', array('room' => $room['roomid'], 'date'=>$rdates[$m]));?>" target="_blank" title="点击这里在此日期申请此房间"><?php
